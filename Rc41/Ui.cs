...
//                   name    cmd  scmd     kc   skc  bit      alpha     salpha
            ...
            new Key("44",    999,   41,  0x44, 0x4c,   1,      K_BS,     K_CLA ),      // 17  BS     CLX     bs      cla
        ...
        public void AddNumber(char n)
        {
            ...
            if (!cpu.FlagSet(Cpu.F_PRGM))  cpu.SetFlag(Cpu.F_NUM_IN);
            if (cpu.FlagSet(Cpu.F_SYS) == false)
            {
        ...
        string Display()
        {
            ...
            if (mode == M_BASE && cpu.FlagSet(Cpu.F_ALPHA))
            {
                ...
                if (cpu.FlagSet(Cpu.F_SYS)) line += "_";
                return line;
            }
            ...
            if (cpu.FlagSet(Cpu.F_SYS) && cpu.FlagSet(Cpu.F_ALPHA) && mode != M_NARG)
            {
                i = 0;
        ...
        public void ProgramAlphaBack()
        {
            int addr;
            int ln;
            if (!cpu.FlagSet(Cpu.F_SYS)) return;
        ...
        public void AlphaUp(int key)
        {
            ...
            if (mode == M_BASE)
            {
                ...
                if (key == K_BS)                     // BS
                {
                    if (cpu.FlagSet(Cpu.F_PRGM) && cpu.FlagSet(Cpu.F_ALPHA) && cpu.FlagSet(Cpu.F_SYS))
                    {
                        ProgramAlphaBack();
                        return;
                    }
                    if (cpu.FlagSet(Cpu.F_ALPHA) &&cpu.FlagSet(Cpu.F_SYS))
                    {
                        flag = false;
                        for (i = Cpu.REG_M; i <= Cpu.REG_P + 2; i++)
                            if (cpu.ram[i] != 0x00) flag = true;
                        if (!flag) cpu.ClearFlag(Cpu.F_SYS);
        ...
        public void KeyUp(int key)
        {
            ...
            if (mode == M_NARG)
            {
                ...
                else if (cmd == 10 && (cpu.catalog[cpu.ram[Cpu.REG_R + 0]].flags & N_ST) != 0)
                {
                ...
                else if (cmd < 10)
                {
                    if ((cpu.ram[Cpu.REG_R + 2] & 0x0f) == 0x0f)
                        cpu.ram[Cpu.REG_R + 2] = (byte)((cpu.ram[Cpu.REG_R + 2] & 0xf0) | (uint)cmd);
                    else if ((cpu.ram[Cpu.REG_R + 1] & 0xf0) == 0xf0)
                        cpu.ram[Cpu.REG_R + 1] = (byte)((cpu.ram[Cpu.REG_R + 1] & 0x0f) | ((uint)cmd << 4));
                    else
                        cpu.ram[Cpu.REG_R + 1] = (byte)((cpu.ram[Cpu.REG_R + 1] & 0xf0) | (uint)cmd);
                }
                else if (cmd == 10)
                {
                    if (cpu.ram[Cpu.REG_R + 1] != 0xff) return;
                ...
                if ((cpu.ram[Cpu.REG_R + 1] & 0x0f) != 0x0f)
                {
                    ...
                    if (cpu.ram[Cpu.REG_R+0] == Cpu.CMD_CAT)
                    {
                        ...
                        cpu.ram[Cpu.REG_R + 2] = (byte)((cpu.ram[Cpu.REG_R + 2] & 0xf0) | b);
//                        cpu.ram[Cpu.REG_R + 0] = b;
                        cpu.ram[Cpu.REG_R + 0] = 0;
        ...
        public void XeqAlphaArg()
        {
            ...
            if (cmd == cpu.CMD_GTO_ALPHA)
            {
                if (cpu.FlagSet(Cpu.F_PRGM))
                {
                    if (buffer.Length == 1)
                    {
                        ...
                        {
                            b = (byte)(buffer[0] + 37);
                            cpu.ram[Cpu.REG_R + 1] = 0xd0;
                            cpu.ram[Cpu.REG_R + 0] = 0x00;
                            cpu.ram[Cpu.REG_E + 2] &= 0xf0;
                            cpu.ram[Cpu.REG_E + 2] |= (byte)(((b & 0x70) >> 4));
                            cpu.ram[Cpu.REG_E + 1] &= 0x0f;
                            cpu.ram[Cpu.REG_E + 1] |= (byte)(((b & 0x0f) << 4));
                            cpu.Execute();
                            return;
                        }
                        if (buffer[0] >= 'a' && buffer[0] <= 'e')
            ...
            if (cmd == cpu.CMD_LBL_A)
            {
                if (buffer.Length == 1)
                {
                    if (buffer[0] >= 'A' && buffer[0] <= 'J')
                    {
                        cpu.ram[Cpu.REG_R + 1] = 0xcf;
                        cpu.ram[Cpu.REG_R + 0] = (byte)(buffer[0] + 37);
                        cpu.Execute();
                        return;
                    }
                    if (buffer[0] >= 'a' && buffer[0] <= 'e')
                    {
                        cpu.ram[Cpu.REG_R + 1] = 0xcf;
                        cpu.ram[Cpu.REG_R + 0] = (byte)(buffer[0] + 26);
                        cpu.Execute();
                        return;
                    }
                }
        ...
        {
            if (cpu.calculatorMode == Cpu.CM_SW) return;
            if (cpu.calculatorMode == Cpu.CM_CLOCK) cpu.timeModule.EndClock();
            if (cpu.dirMode) return;
        ...
        public void ButtonDown(string tag)
        {
            ...
            if ((cpu.FlagSet(Cpu.F_ALPHA) && mode != M_NARG) || (cpu.FlagSet(Cpu.F_SYS) && cpu.FlagSet(Cpu.F_ALPHA) && mode != M_NARG)) AlphaDown(key);
            else pendingKey = key;
        }
        ...
        public void ButtonUp(string tag)
        {
            if (cpu.calculatorMode == Cpu.CM_SW)
            {
                cpu.timeModule.KeyUp(tag);
                return;
            }
            if (cpu.dirMode) return;
            if (pendingKey < 0) return;
            if ((cpu.FlagSet(Cpu.F_ALPHA) && mode != M_NARG) || (cpu.FlagSet(Cpu.F_SYS) && cpu.FlagSet(Cpu.F_ALPHA) && mode != M_NARG)) AlphaUp(pendingKey);
